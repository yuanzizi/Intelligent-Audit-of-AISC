    ...
    with tf.python_io.TFRecordWriter(RECORD_NAME) as writer:
        for label_true, names in all_names.items():
            for name in names:
                ...
                byte_img = byte_img.resize((300,300))
...
def read_copy():
    ...
    with tf.Session() as sess:
        sess.run(init_op)
        saver.restore(sess, os.path.join('..','..','model_300','model1.ckpt')) 
...
#    print(folder)
    ...
    print(folder, len(img_list))
    if folder in ['6','7','8']:
#        continue
        img_names[int(folder)] = img_list[:200]
    else:
        img_names[int(folder)] = img_list[:200]
#        pass
